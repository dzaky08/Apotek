...
namespace projek_apotek.uc
{
    public partial class ucTransaksi : UserControl
    {
        ...
        private void autoCompleted()
        {
            try
            {
                var dt = Connect.getData("SELECT * FROM obat ORDER BY nama_obat ASC");
        ...
        {
            if(transaksi.keranjang.Count > 0)
            {
                BindingSource bs = new BindingSource();
                bs.DataSource = transaksi.keranjang;
                dgKeranjang.DataSource = bs;
                lblTotal.Text = transaksi.keranjang.Sum(x => x.total_harga).ToString();
        ...
        {
            int total = transaksi.keranjang.Sum(x => x.total_harga);
